...
public class RigidbodyPickUp : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (throwingSystem.aboutToThrow)
        {
            ...
            throwingSystem.objectToThrow.GetComponent<Rigidbody>().useGravity = true;
            //Applies force to the objectt that was applied
            throwingSystem.objectToThrow.GetComponent<Rigidbody>().AddForce(playerAim.direction * throwingSystem.strength);
        ...
        {
            Color alpha = objectHoldingOpacity.alphaObject.GetComponent<Renderer>().material.color;
            alpha.a = 1f;
            objectHoldingOpacity.alphaObject.GetComponent<Renderer>().material.color = alpha;
